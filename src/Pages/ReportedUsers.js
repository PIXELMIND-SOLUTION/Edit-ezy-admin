...
export default function ReportedUsers() {
  ...
  const fetchReportedUsers = async () => {
    try {
      const res = await axios.get("https://api.editezy.com/api/admin/getallreporteduser");
      if (res.data?.users) {
  ...
  const handleBlock = async (userId, isBlocked) => {
    ...
    try {
      const response = await axios.put(`https://api.editezy.com/api/admin/block-user/${userId}`, {
        block: !isBlocked,
      });
