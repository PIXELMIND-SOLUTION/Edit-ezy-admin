...
export default function BusinessCardList() {
  ...
  const fetchBusinessCards = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://31.97.228.17:4061/api/admin/getbusinesscards");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://31.97.228.17:4061/api/admin/deletebusinesscard/${id}`);
      fetchBusinessCards();
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `http://31.97.228.17:4061/api/admin/updatebusinesscard/${editData._id}`,
        editData
      );
