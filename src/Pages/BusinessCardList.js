...
export default function BusinessCardList() {
  ...
  const fetchBusinessCards = async () => {
    setLoading(true);
    try {
      const res = await axios.get("https://api.editezy.com/api/admin/getbusinesscards");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://api.editezy.com/api/admin/deletebusinesscard/${id}`);
      fetchBusinessCards();
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `https://api.editezy.com/api/admin/updatebusinesscard/${editData._id}`,
        editData
      );
