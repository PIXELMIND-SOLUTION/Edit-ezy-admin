...
export default function LogoPage() {
  ...
  const fetchLogos = async () => {
    setLoading(true);
    try {
      const res = await axios.get("https://api.editezy.com/api/admin/getlogos");
      setLogos(res.data || []);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://api.editezy.com/api/admin/deletelogo/${id}`);
      fetchLogos();
    } catch (error) {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `https://api.editezy.com/api/admin/updatelogo/${editData._id}`,
        editData
      );
      setModalOpen(false);
      fetchLogos();
