...
const GetAboutUsPage = () => {
  ...
  useEffect(() => {
    const fetchAboutUsData = async () => {
      try {
        const response = await axios.get("https://api.editezy.com/api/admin/getaboutus");
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      await axios.put(
        `https://api.editezy.com/api/admin/updateaboutus/${aboutUsData._id}`,
        { title, content, date }
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete(
        `https://api.editezy.com/api/admin/deleteaboutus/${aboutUsData._id}`
      );
